...
class Pinger {
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                HttpProxyCacheDebugger.printfWarning("Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
            } catch (InterruptedException | ExecutionException e) {
                HttpProxyCacheDebugger.printfError("Error pinging server due to unexpected error", e);
            }
            attempts++;
            timeout *= 2;
        }
        ...
        HttpProxyCacheDebugger.printfError(error, new ProxyCacheException(error));
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        try {
            ...
            HttpProxyCacheDebugger.printfLog("Ping response: `" + new String(response) + "`, pinged? " + pingOk);
            return pingOk;
        } catch (ProxyCacheException e) {
            HttpProxyCacheDebugger.printfError("Error reading ping response", e);
            return false;
        } finally {
            source.close();
        }
