...
import com.danikula.videocache.HttpProxyCacheDebugger;
        ...
        for (File file : files) {
            ...
            if (!accepted) {
                ...
                if (deleted) {
                    totalCount--;
                    totalSize -= fileSize;
                    HttpProxyCacheDebugger.printfLog("Cache file " + file + " is deleted because it exceeds cache limit");
                } else {
                    HttpProxyCacheDebugger.printfError("Error deleting file " + file + " for trimming cache");
                }
            }
        }
    }
