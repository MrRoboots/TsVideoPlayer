package com.robot.tsvideoplayer.activity.api
...
import com.robot.tsplayer_kotlin.player.VideoView
import com.robot.tsplayer_kotlin.utils.L
import com.robot.tsplayer_ui.StandardVideoController
import com.robot.tsplayer_ui.component.*
import com.robot.tsvideoplayer.R
import com.robot.tsvideoplayer.activity.BaseActivity
import com.robot.tsvideoplayer.util.IntentKeys
import com.robot.tsvideoplayer.util.Utils
...
class PlayerActivity : BaseActivity<VideoView>() {
    ...
    override fun initView() {
        super.initView()
        mVideoView = findViewById<VideoView>(R.id.player)
        intent?.let {
            ...
            val thumb = prepareView.findViewById<ImageView>(com.robot.tsplayer_ui.R.id.thumb) //封面图
...
//            mVideoView.skipPositionWhenPlay(10000)
            ...
            override fun onPlayStateChanged(playState: Int) {
                when (playState) {
                    ...
                    BaseVideoView.STATE_PLAYING -> {
                        ...
                        L.d("视频宽：" + (videoSize?.get(0) ?: 0))
                        L.d("视频高：" + (videoSize?.get(1) ?: 0))
                    }
    ...
    fun onButtonClick(view: View) {
        /*  when (view.id) {
              R.id.scale_default -> mVideoView!!.setScreenScaleType(BaseVideoView.SCREEN_SCALE_DEFAULT)
              R.id.scale_169 -> mVideoView!!.setScreenScaleType(BaseVideoView.SCREEN_SCALE_16_9)
              R.id.scale_43 -> mVideoView!!.setScreenScaleType(BaseVideoView.SCREEN_SCALE_4_3)
              R.id.scale_original -> mVideoView!!.setScreenScaleType(BaseVideoView.SCREEN_SCALE_ORIGINAL)
              R.id.scale_match_parent -> mVideoView!!.setScreenScaleType(BaseVideoView.SCREEN_SCALE_MATCH_PARENT)
              R.id.scale_center_crop -> mVideoView!!.setScreenScaleType(BaseVideoView.SCREEN_SCALE_CENTER_CROP)
    ...
    override fun onPause() {
        super.onPause()
        //如果视频还在准备就 activity 就进入了后台，建议直接将 BaseVideoView release
        //防止进入后台后视频还在播放
        if (mVideoView?.mCurrentPlayState == BaseVideoView.STATE_PREPARING) {
            mVideoView?.release()
        }
    }
