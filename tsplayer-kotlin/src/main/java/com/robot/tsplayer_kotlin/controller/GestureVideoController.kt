...
abstract class GestureVideoController : BaseVideoController, GestureDetector.OnGestureListener,
    ...
    override fun onScroll(e1: MotionEvent, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
        ...
        if (mFirstTouch) {
            ...
            if (mChangePosition || mChangeBrightness || mChangeVolume) {
                for (component: Map.Entry<IControlComponent, Boolean> in mControlComponents) {
                    if (component.key is IGestureComponent) {
                        (component.key as IGestureComponent).onStartSlide()
                    }
                }
            }
    ...
    private fun slideToChangeVolume(deltaY: Float) {
        ...
        for (component: Map.Entry<IControlComponent, Boolean> in mControlComponents) {
            if (component.key is IGestureComponent) {
                (component.key as IGestureComponent).onVolumeChange(percent)
            }
        }
    }
    ...
    private fun slideToChangeBrightness(deltaY: Float) {
        ...
        for (component: Map.Entry<IControlComponent, Boolean> in mControlComponents) {
            if (component.key is IGestureComponent) {
                (component.key as IGestureComponent).onBrightnessChange(percent)
            }
        }
    }
    ...
    private fun slideToChangePosition(deltaX: Float) {
        ...
        for (component: Map.Entry<IControlComponent, Boolean> in mControlComponents) {
            if (component.key is IGestureComponent) {
                (component.key as IGestureComponent).onPositionChange(pos, currentPosition, duration)
            }
        }
        mSeekPosition = pos
    }
    ...
    private fun stopSlide() {
        for (component: Map.Entry<IControlComponent, Boolean> in mControlComponents) {
            if (component.key is IGestureComponent) {
                (component.key as IGestureComponent).onStopSlide()
            }
        }
    }
